TH_FULL_MONTHS_MAPPER = {
    "มกราคม": 'January',
    "กุมภาพันธ์": 'February',
    "มีนาคม": 'March',
    "เมษายน": 'April',
    "พฤษภาคม": 'May',
    "มิถุนายน": 'June',
    "กรกฎาคม": 'July',
    "สิงหาคม": 'August',
    "กันยายน": 'September',
    "ตุลาคม": 'October',
    "พฤศจิกายน": 'November',
    "ธันวาคม": 'December',
}

TH_FULL_MONTHS_MAPPER_MGR = {
    "ม.ค.": 'January',
    "ก.พ.": 'February',
    "มี.ค.": 'March',
    "เม.ย.": 'April',
    "พ.ค.": 'May',
    "มิ.ย.": 'June',
    "ก.ค.": 'July',
    "ส.ค.": 'August',
    "ก.ย.": 'September',
    "ต.ค.": 'October',
    "พ.ย.": 'November',
    "ธ.ค.": 'December',
}

DAILYNEWS_CATEGORY_MAPPER = {
    "เศรษฐกิจ-ยานยนต์" : "economic","เศรษฐกิจจานร้อน" : "economic","มุมคนเมือง" : "economic","ป้ายแดงชวนขับ" : "economic",
    "รวมข่าวโควิด-19" : "economic", "ธุรกิจ" : "economic",
    "พาหนะคู่ใจ" : "economic","รู้ก่อนเหยียบ" : "economic",
    "การเมือง" : "politics","ที่เห็นและเป็นอยู่" : "politics","คอลัมนิสต์" : "politics","เลียบค่ายกองทัพ" : "politics",
    "ตรวจการบ้าน" : "politics","ว่ายทวนน้ำ" : "politics",
    "เกษตร" : "agriculture",
    "ทั่วไทย-กทม." : "thailand","คนดีของสังคม" : "thailand","นิยายชีวิต" : "thailand","Special Report" : "thailand", "ทั่วไทย" : "thailand",
    "สังคมภูมิภาค" : "thailand","ส่องพระยอดนิยม" : "thailand",
    "การศึกษา-ไอที" : "education-it","การศึกษา" : "education-it","ไอที" : "education-it",
    "ท้องถิ่น" : "local", "ช่องทางทำกิน" : "local", "รายงานพิเศษ" : "local",
    "กีฬา" : "sports","Crazy Football" : "sports","สังคมกอล์ฟ" : "sports", "กอล์ฟ" : "sports", "กอล์ฟไทย" : "sports",
    "E-Sport" : "sports", "กัลโช เซเรีย อา" : "sports", "Sports Zone" : "sports", "ทีมชาติไทย" : "sports", "กีฬาไทย" : "sports",
    "สุขภาพ-ความงาม" : "health-beauty",
    "อาชญากรรม" : "crime","หนอนโรงพัก" : "crime","สน.รอตรวจ" : "crime","เกาะประเด็นดัง" : "crime",
    "บันเทิง" : "entertainment","ดูหนังกับหมี" : "entertainment","1 Day with ซุปตาร์" : "entertainment","มายาลั้ลลา" : "entertainment",
    "บันเทิงต่างประเทศ" : "entertainment",
    "ต่างประเทศ" : "foreign","ผ่าโลกวันอาทิตย์" : "foreign","สารพันต่างแดน" : "foreign",
    "ที่นี่จากยูเอสเอ" : "foreign","คนไทยในออสเตรเลีย" : "foreign"
}

BANGKOKBIZNEWS_CATEGORY_MAPPER = {
    "เศรษฐกิจ" : "economic",
    "การเมือง" : "politics",
    "การเงิน-การลงทุน" : "finance",
    "ธุรกิจ" : "business",
    "ต่างประเทศ" : "foreign",
    "bizweek" : "bizweek",
    "คุณภาพชีวิต-สังคม" : "life",
    "จุดประกาย" : "judprakai",
    "ไลฟ์สไตล์" : "lifestyle",
    "ข่าวประชาสัมพันธ์" : "pr",
    "ข่าวพระราชสำนัก" : "royal",
    "อาชญากรรม" : "crime",
    "ทั่วไทย" : "local",
    "บทวิเคราะห์" : "analysis",
    "EEC Focus" : "eec",
    "Big Data Analysis" : "bigdataanalysis",
    "ข้อมูลเศรษฐกิจ" : "data",
    "ยานยนต์" : "auto",
    "นวัตกรรม" : "innovation",
    "ไอที-ไอเดีย" : "it",
    "อสังหาริมทรัพย์" : "property",
}

MATICHON_CATEGORY_MAPPER = {
    "เศรษฐกิจ" : "economy", "รายงานเศรษฐกิจ" : "economy",
    "การศึกษา" : "education", "ศาสนา-วัฒนธรรม" : "education", "มุมบริการ" : "education",
    "การเมือง" : "politics","รายงาน-วิเคราะห์" : "politics", "สัมภาษณ์พิเศษ" : "politics","เรียงคน/ภาพข่าวสังคม" : "politics",
    "เลือกตั้ง 62" : "politics",
    "หุ้น-การเงิน" : "stock-finance", "ภาวะหุ้นรายวัน" : "daily-stock-markets", "ตลาดหุ้น" : "stock-markets",
    "ธุรกิจ" : "business",
    "ต่างประเทศ" : "foreign",
    "ไลฟ์สไตล์" : "lifestyle","สุขภาพ-ความงาม" : "lifestyle",
    "สังคม-สตรี" : "lifestyle","ดวง-โหราศาสตร์" : "lifestyle","อาหาร-ท่องเที่ยว" : "lifestyle","เด็ก-เยาวชน" : "lifestyle",
    "ข่าวพระราชสำนัก" : "royal",
    "อาชญากรรรม-กระบวนการยุติธรรม" : "crime", "อาชญากรรรม" : "crime",
    "ในประเทศ" : "local",
    "บทวิเคราะห์" : "analysis",
    "คอลัมนิสต์" : "columnists",
    "วิทยาการ-ไอที" : "tech",
    "นิวส์มอนิเตอร์" : "news-monitor",
    "รถยนต์" : "auto",
    "ภูมิภาค" : "region",
    "เกาะกระแสโซเชียล" : "social", 
    "บันเทิง" : "entertainment", "บันเทิงเทศ" : "entertainment","บันเทิงไทย" : "entertainment", 'ศิลปะและวัฒนธรรม' : "entertainment", "วิจารณ์บันเทิง" : "entertainment",
    "ชีวิตคุณภาพ" : "quality-life",
    "บทความ" : "article", "สถานีคิดเลขที่ 12" : "article", "เดินหน้าชน" : "article", "คุณภาพคือความอยู่รอด" : "article",
    "คอลัมน์หน้า 3 มติชน" : "article",
    "เกาะติด Covid-19" : "covid19", "โควิด-ไทย" : "covid19","ด่วน โควิด 19" : "covid19",
    "กีฬา" : "sport", "เจาะลึกแดนอาทิตย์อุทัย" : "sport", "เกาะติดโตเกียวเกมส์" : "sport", "Sport Scoops" : "sport",
    "ซีเกมส์ 2022" : "sport", "เกาะติดฮานอยเกมส์" : "sport", "ข่าวกีฬาเด็ด" : "sport", 'อินไซด์ฮานอย': 'sport',
    "กีฬาต่างประเทศ" : "sport", "กีฬาในประเทศ" : "sport", "ฟุตบอลต่างประเทศ" : "sport", "ฟุตบอลในประเทศ" : "sport",
    "Sport Slide" : "sport", "โตเกียวเกมส์" : "sport", "มวย" : "sport", "เกาะติดยูโร" : "sport", "ฟุตบอลยุโรป" : "sport",
    "ยูโร2020" : "sport", "ฟุตบอลอังกฤษ" : "sport", "สกู๊ปยูโร2020" : "sport"
}

MANGERONLINE_CATEGORY_MAPPER = {
    "qol" : "quality-life",
    "around" : "foreign",
    "crime" : "crime",
    "sport" : "sports",
    "science" : "science",
    "uptodate" : "uptodate",
    "cyberbiz" : "cyberbiz",
    "motoring" : "motoring",
    "stockmarket" : "stockmarket",
    "business" : "business",
    "greeninnovation" : "greeninnovation",
    "columnist" : "columnist",
    "local" : "local",
    "politics" : "politics",
    "smes" : "smes",
    "travel" : "travel",
    "entertainment" : "entertainment",
    "game" : "game",
    "drama" : "drama",
    "china" : "china",
    "south" : "south"
}

PRACHACHAT_CATEGORY_MAPPER = {
    "finance" : 'หุ้น-การเงิน',
    "marketing": 'การตลาด',
    "economy": 'เศรษฐกิจในประเทศ',
    "property": 'อสังหาริมทรัพย์',
    "politics": 'การเมือง',
    "world-news": 'ต่างประเทศ',
    "motoring": 'รถยนต์',
    "ict": 'ไอซีที',
    "tourism": 'ธุรกิจท่องเที่ยว',
    "csr-hr": 'ซีเอสอาร์-เอชอาร์',
    "d-life": 'ดีไลฟ์'
}

SUB_CATEGORY_DELETE_MATICHON = ['Hot Shot! อินแจแปน','คลิปลุยโตเกียว 2020','แฟชั่น','เด่นวันนี้','การ์ตูนอรุณ วัชระสวัสดิ์', 'มติชนทีวี', 'วิเคราะห์',
                            'ข่าวในพระราชสำนัก','Preview ประจำวัน','ประชาชื่น', 'สถานการณ์โลก','ปริทรรศน์โลก', 'ความเคลื่อนไหว', 'International',
                            'ข่าวประชาสัมพันธ์', 'แกลลอรียูโร2020','ภาพนำ','ปริทรรศน์โลก','Uncategorized','การ์ตูนรุทธ์','วงวรรณกรรม', 'ซีเกมส์ 2019',
                            'คลิปรายการ','ชโลทร','แวดวงหนังสือ','ประชาชื่น','สัมภาษณ์','คอลัมน์ประจำ','สกู๊ปประชาชื่น', 'ผู้ว่าฯกทม.', 'SEA Games 2019',
                            'ศึกชิงกทม.-พัทยา 65', 'นายกเมืองพัทยา', 'สมาชิกเมืองพัทยา', 'สัมภาษณ์พิเศษ','หน้า 1 หนังสือพิมพ์', 'โพล - สำรวจความเห็น', 
                            'นิวส์รูมวิเคราะห์', 'Book Slide', 'แกลเลอรี่ซีเกมส์', 'บัตรเครดิต', 'บริหารการเงิน', 'ข่าว']

CATEGORY_DELETE_MATICHON = ['book', 'sme',]

CATEGORY_TH_DELETE_MATICHON = ['คอลัมน์-รายงานพิเศษ']

CATEGORY_DELETE_THESTANDARD = ['เมธาพันธุ์วราทร','ไม่มีหมวดหมู่', 'คำนี้ดี', 'Exclusive Content', 'Wealth Alert', 'Now & Next 2021', 
                                'วีรวัฒน์อัจจุตมานัส', 'Book', 'News Now and Next 2022']

CATEGORY_DELETE_DAILYNEWS = ['อื่นๆ','ก้อนเมฆเล่าเรื่อง','การ์ตูนขวด','นวัตกรรม','ดวง-หวย', 'กทม.']

MANAGER_DELETE_TAGS = ['MGRTOP7', 'MGROnline']

NEWS_MAX_NUM_PER_PAGE = 10